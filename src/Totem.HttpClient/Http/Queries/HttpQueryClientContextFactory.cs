...
public class HttpQueryClientContextFactory : IHttpQueryClientContextFactory
{
    delegate IHttpQueryClientContext<IHttpQuery> TypeFactory(Id pipelineId, IHttpQueryEnvelope envelope);

    readonly ConcurrentDictionary<Type, TypeFactory> _factoriesByQueryType = new();

    public IHttpQueryClientContext<IHttpQuery> Create(Id pipelineId, IHttpQueryEnvelope envelope)
    {
        if(envelope is null)
    ...
    TypeFactory CompileFactory(Type queryType)
    {
        ...
        var constructor = typeof(HttpQueryClientContext<>).MakeGenericType(queryType).GetConstructors().Single();
