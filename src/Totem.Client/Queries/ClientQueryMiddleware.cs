...
namespace Totem.Queries
{
    ...
    {
        readonly Func<IClientQueryContext<IHttpQuery>, Func<Task>, CancellationToken, Task> _middleware;

        public ClientQueryMiddleware(Func<IClientQueryContext<IHttpQuery>, Func<Task>, CancellationToken, Task> middleware) =>
            _middleware = middleware ?? throw new ArgumentNullException(nameof(middleware));

        public Task InvokeAsync(IClientQueryContext<IHttpQuery> context, Func<Task> next, CancellationToken cancellationToken) =>
