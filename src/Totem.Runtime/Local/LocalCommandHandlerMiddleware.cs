...
{
    public class LocalCommandHandlerMiddleware : ILocalCommandMiddleware
    {
        delegate ILocalCommandHandler<ILocalCommand> TypeFactory(IServiceProvider services);

        readonly ConcurrentDictionary<Type, TypeFactory> _factoriesByCommandType = new();
        readonly ILogger _logger;
        readonly IServiceProvider _services;

        public LocalCommandHandlerMiddleware(ILogger<LocalCommandHandlerMiddleware> logger, IServiceProvider services)
        {
        ...
        async Task TryInvokeHandlerAsync(ILocalCommandContext<ILocalCommand> context, CancellationToken cancellationToken)
        {
        ...
        class TypedHandler<TCommand> : ILocalCommandHandler<ILocalCommand> where TCommand : ILocalCommand
        {
            readonly IServiceProvider _services;
            ...
            public async Task HandleAsync(ILocalCommandContext<ILocalCommand> context, CancellationToken cancellationToken)
            {
                var handler = _services.GetService<ILocalCommandHandler<TCommand>>();

                if(handler != null)
                {
                    await handler.HandleAsync((ILocalCommandContext<TCommand>) context, cancellationToken);
                }
            }
        }
    }
