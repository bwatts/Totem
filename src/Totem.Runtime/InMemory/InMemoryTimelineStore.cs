...
{
    public class InMemoryTimelineStore : ITimelineStore
    {
        readonly ConcurrentDictionary<Id, TimelineHistory> _historiesById = new();
        readonly IInMemoryEventSubscription _subscription;
        readonly IClock _clock;

        public InMemoryTimelineStore(IInMemoryEventSubscription subscription, IClock clock)
        {
        ...
        class TimelineHistory
        {
            ...
            internal IEnumerable<IEventEnvelope> Save(ITimeline timeline, Id correlationId, ClaimsPrincipal principal, DateTimeOffset whenOccurred)
            {
                ...
                foreach(var newEvent in timeline.NewEvents)
                {
                    ...
                    var envelope = new EventEnvelope(eventId, newEvent, EventInfo.From(newEvent), correlationId, principal, TimelineType, TimelineId, version, whenOccurred);
