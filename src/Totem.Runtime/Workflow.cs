...
namespace Totem
{
    public abstract class Workflow : EventSourced, IWorkflow
    {
        ...
        {
            if(HasErrors)
                throw new InvalidOperationException($"Workflow {this} has one or more errors preventing {envelope.Info.MessageType.Name} from enqueueing on {envelope.Info.QueueName}");
